...
namespace StreamMgr
{
    ...
    public:
		...
		//-----------------------------------------------------------------------------
		inline ScePthread BlockedThreadID() { return m_idBlockedThread; }

	private:
        ScePthread	m_idBlockedThread;	// ID of thread that is waiting for I/O to complete.	
    };
	...
	private:
		...
		// IO thread semaphore.
		ScePthreadMutex		m_mutexPendingStmsSem;	// Mutex: used for scheduler control critical section.
		ScePthreadCond		m_condAreTasksPending;	// Conditional variable. If the condition (read inside the critical section) for 
		...
		ScePthreadMutex		m_mutexBlockingIO;
		ScePthreadCond		m_condBlockingIODone;
